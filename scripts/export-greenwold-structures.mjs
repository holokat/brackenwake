...
md.push('');
md.push(`Generated ${date} by scripts/export-greenwold-structures.mjs from the ${spaces.length} greenwold spaces in src/mmo/spaces, the guide's per zone lists, src/mmo/plans/footprints.js and public/models/props/manifest.json. ${distinct.length} distinct structures: ${nMade} made, ${nToMake} to make${nNoFoot ? `, ${nNoFoot} with no footprint row (never loaded until one is added)` : ''}. Sizes are the base library FOOTPRINT, width by depth by height. A placed piece multiplies these dimensions by its saved scale; the authored settlement buildings currently use 1.25. The words and prompts are MODELS.md's; the spec (glb, metres, Y up, budgets by height, validator) is at the top of that file.`);
md.push('');
