...
- `tools/boot_screen.mjs` includes `src/game/ui/boot-screen.js` directly in the
  entry HTML before the deferred game module runs. The loader can report a
  failed module download without needing another script request.
...
- Actual startup failures keep the same artwork, show a retry button, and put
  the first diagnostic in a disclosure using text content, never HTML.
...
available to inspect. Controls exercise loading, failure, details, retry,
