...
- Outside Old Cellars, movement toward the entrance now prepares only the first-floor arrival GLB. A five-per-second check starts a low-priority byte download within 140 metres when the player is heading toward the door with an estimated arrival within 25 seconds. Standing within 18 metres also qualifies. Walking past, moving away and teleporting across the map do not project an approach.
- Within 45 metres and an estimated eight seconds of entry, the approach loader can decode that one model through the shared serial queue. It defers speculative decoding while measured frame work exceeds 14 ms. Background acquisition still reserves a network slot for visible assets. Turning away for three seconds releases the preload; leaving the area, hiding the tab or enabling data saving also cancels unnecessary work. Completed resources remain subject to the existing cache limits.
- Entry takes a high-priority lease before releasing the surface preload, so both completed and unfinished downloads survive the handoff. It releases that lease when the actual nearby room set settles. This prepares a model, not a dungeon scene: room instances, collision shells, lighting and shader preparation still belong to entry, and no exploration or minimap state is changed outside.
...
The runtime exposes `window.__bw.assetLoading`, `window.__bw.runtime.dungeonPreload` and `window.__bw.runtime.dungeonScene.streaming.stats`. The surface preload reports its selected site, distance, phase, preparation time and the last entry handoff, including whether the model was ready before entry. Both asset-loading and Cellars-exterior review pages include these fields in their snapshots. `built.ready` now waits for the currently nearby room set, not the entire floor. All room leaf APIs retain `ready`, `loaded`, load injection and idempotent disposal.

`node --test src/game/streaming/dungeon_approach.test.mjs` verifies approach direction, throttling, frame-budget deferral, single-download handoff, cancellation, data-saving restrictions, explicit-entry priority and retry backoff. Its real-asset check parses the 21,967,680-byte arrival GLB with 14 meshes and verifies reuse across entry with one fetch and one parse. Node's image stubs do not measure browser image decoding, GPU work or player-visible load time. The runtime suite also walks the actual authored island entrance, verifies the preload starts outside without creating a dungeon, and checks transition lease ordering and release.
