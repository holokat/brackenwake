...
const included=new Set(['weapon','armour','shield','offhand','instrument','jewellery']);
export const itemCatalog=[
 ...wiki.bases.filter(b=>b.kind!=='shield'&&(included.has(b.kind)||tools.has(b.id)||ammunition.has(b.id))).map(base=>({
  ...base,kind:base.kind==='armour'?'armor':base.kind==='instrument'?'offhand':tools.has(base.id)?'tool':ammunition.has(base.id)?'ammunition':base.kind,
  family:base.kind==='armour'?base.material:base.kind==='weapon'?(wiki.weapons.find(w=>w.id===base.id)?.skill||'magery'):base.kind,
...
const kindOrder=['weapon','shield','armor','offhand','tool','jewellery','ammunition','material'];
...
export const slotLabels={head:'Head',chest:'Chest',hands:'Hands',wrists:'Wrists',waist:'Waist',legs:'Legs',feet:'Feet',back:'Back'};
export const categoryOptions=[['all','All items'],['weapon','Weapons'],['armor','Armor'],['shield','Shields'],['offhand','Books and offhands'],['tool','Tools'],['jewellery','Jewelry'],['ammunition','Ammunition'],['material','Materials']];
...
export function defaultMaterial(item){
 ...
 if(['leather','studded'].includes(item.material))return 'hide';
 if(['ring','chain','plate'].includes(item.material))return 'iron';
 if(/bow|staff|wand|lute|torch|arrow|bolt/.test(item.id))return 'oak';
 return item.kind==='armor'?null:'iron';
}
export function itemDetail(item){
 ...
 if(item.kind==='armor')return `${wiki.tiers.find(t=>t.id===item.material)?.material||item.material} · ${slotLabels[item.slot]}`;
