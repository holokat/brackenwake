...
export function poseArchery(rig,item,{move='reference',phase=0}={}){
 ...
 const anchor=new THREE.Vector3(.50,-.30,.035).multiplyScalar(bodyScale).applyQuaternion(headAim).add(head.getWorldPosition(new THREE.Vector3()));
 const fullDistance=rig.bodyType==='female'?2.71:2.84;
 const moving=/walk|run|jump/.test(move);
 const carry=new THREE.Vector3(moving?-1.9:-1.73*bodyScale,moving?-.75:-.94,moving?-1.55:-1.74).applyQuaternion(frame).add(origin);
 ...
 const aimGrip=anchor.clone().addScaledVector(aim,fullDistance-1.45*(1-extension)).addScaledVector(vertical,-.30);
 const palm=carry.lerp(aimGrip,state.raise);
 palm.add(new THREE.Vector3(0,-.72*Math.sin(Math.PI*state.raise)*(1-state.draw),0).applyQuaternion(frame));
 item.userData.twoHandedGrip.left=[0,0,-.30];
 // The grip is the fixed pivot. The drawn string travels toward the facial anchor.
 const position=palm.clone().sub(new THREE.Vector3(0,0,-.30).applyQuaternion(rotation));
 const restBrace=-item.userData.bowConstruction.center[0],drawDistance=fullDistance-restBrace;
 ...
 poseBowArrow(item,{visible:phase>=.09&&phase<BOW_RELEASE_PHASE&&state.stage!=='carry',load:state.load,length:fullDistance+.48});
