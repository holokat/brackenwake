...
// ============================================================= the translation
{
  ...
  check('every material in the map is a real armour tier',
    Object.values(ARMOUR_MATERIAL).every((m) => !!BASES[`${m}_chest`]), Object.values(ARMOUR_MATERIAL).join(', '));
...
// a monster's words to items.js bases rather than through a hand written table.
{
  ...
  check('and the warrior walks off with a longsword the bandit never carried, and visible armour pieces',
    w.bases.longsword > 0 && slotsOf(w.bases).size >= 1 && [...slotsOf(w.bases)].every((slot) => ['head', 'shoulders', 'chest', 'hands', 'waist', 'legs', 'feet'].includes(slot)),
