...
console.log('net: the room and the hello');
{
  ...
  const c = { id: 'ranger-x1', name: 'Bob', opening: 'ranger', appearance: { skin: 2 }, equipment: { mainHand: { base: 'shortbow' }, offHand: { base: 'dagger' }, chest: { base: 'leather_chest' } } };
  const h = helloFor(c);
  check('the hello carries id, name and the look another client needs', h.t === 'hello' && h.id === 'ranger-x1' && h.name === 'Bob' && h.look.opening === 'ranger' && h.look.gear.mainHand === 'shortbow' && h.look.gear.offHand === 'dagger' && h.look.gear.chest === 'leather_chest' && h.look.gear.neck === null, JSON.stringify(h.look.gear));
