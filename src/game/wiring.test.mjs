...
// ===========================================================================
{
  ...
  check('and the frame order is the nine 07-RUNTIME-CONTRACT.md documents, with the dragon after the world and before the HUD, and the emotes straight after the body they pose',
    FRAME_ORDER.join(',') === 'world,player,emotes,combat,abilities,inventory,world_life,events,dragon,ui,context_menu,dev,input', FRAME_ORDER.join(','));
  ...
  check('the build order is the one R1.md documents',
    sys.built.join(',') === 'world,player,combat,inventory,abilities,ui,emotes,world_life,events,dragon,context_menu,dev,input', sys.built.join(','));
