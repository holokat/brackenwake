import {hasCellarDownStair} from './cellar_stair_model.js';
...
export function furnishCellarDescent(built,L,{load=null,stream,sc}={}){
 ...
 for(const room of L.descent||[]){
  ...
  const proxy=createCellarRoomProxy(spec.colliders,[{polygon:spec.polygon,rx:spec.rx,rz:spec.rz,ceiling:spec.ceiling,ceilingSolid:true,holes:hasCellarDownStair(L)&&room.roomId===9?[{x:0,z:-9,w:6.1,d:6.05}]:[]}]);group.add(proxy.group);
