...
export function furnishShoulder(built,L){
 ...
 const addBox=(key,x,y,z,w,h,d,angle=0)=>{const list=groups.get(key)||[];list.push({x,y,z,w,h,d,angle});groups.set(key,list);};
 ...
 function cart(p,angle,index){const g=new THREE.Group();g.name='Ore wagon';g.position.set(p.x,p.y+.12,p.z);g.rotation.y=angle;root.add(g);
 ...
 const spikes=[];for(const r of L.rooms){for(let i=0;i<18;i++){const gx=r.x+2+(i*13%(r.w-4)),gz=r.z+2+(i*7%(r.h-4));if(!walkable(L,gx,gz))continue;const p=point(gx,gz);const ceiling=ceilingAt(L,gx,gz),length=3+(i%5)*1.4;spikes.push({x:p.x,y:p.y+ceiling-length*.45,z:p.z,length});}}
 ...
 const webPositions=[];
 for(const index of [2,4,6,7,8,10]){const r=L.rooms[index],p=point(r.cx+6,r.cz-5),cy=p.y+5;
 ...
 for(const [index,r] of L.rooms.entries()){const p=point(r.cx+7,r.cz+5);if(index%2===0||index===1){const basket=buildLivingProp('lw_watch_fire');basket.position.set(p.x+.35,p.y,p.z);basket.scale.setScalar(1.8);root.add(basket);emit('watch_fire',{...p,y:p.y+.65},1.1);emit('chimney_embers',{...p,y:p.y+1},1.2);}emit(index%3===0?'mist_bank':'cave_dust',{...p,y:p.y+.2},index%3===0?3:2);if(index===0||index===9)emit('chimney_smoke',{...p,y:p.y+1.6});
 ...
 const lampPositions=[];
 for(const r of L.rooms){const p=point(r.cx+7,r.cz+5);lampPositions.push(new THREE.Vector3(p.x,p.y+3,p.z));}
 ...
 built.dispose=()=>{disposed=true;for(const fx of effects)fx.dispose();dispose();};
 built.mineStats={rooms:L.rooms.length,carts:carts.length,scaffoldDecks:9,rails:groups.get('iron')?.length||0,stalactites:spikes.length};
 return built;
}
