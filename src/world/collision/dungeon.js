...
import {dungeonCameraDistance} from './camera-dungeon.js';
...
export function dungeonPhysical(layout,scene){
 ...
 const index=createCollisionIndex(bodies),canMove=index.canMove,fixtureCeiling=index.ceilingAt,cameraDistance=index.cameraDistance;
 index.cameraDistance=(from,to,r=.28)=>{
  let distance=dungeonCameraDistance(layout,from,to,r,cameraDistance(from,to,r));
  for(const obstacle of scene?.cameraObstacles||[])distance=Math.min(distance,obstacle.distance(from,to,r,distance));
  return distance;
 };
