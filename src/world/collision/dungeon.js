...
export function dungeonPhysical(layout,scene){
 ...
 const index=createCollisionIndex(bodies),canMove=index.canMove;
 index.canMove=(from,to,r,h)=>{const g=gridOf(layout,to.x,to.z);return walkable(layout,g.gx,g.gz)&&canMove(from,to,r,h);};
 index.ceilingAt=(x,z)=>{const g=gridOf(layout,x,z);return floorAt(layout,g.gx,g.gz)+ceilingAt(layout,g.gx,g.gz);};
