...
const combat=createCombat();const field={seed:1,heightAt:()=>0,chunkOf:()=>[0,0],sampleAt:()=>({h:0,biome:'meadow',water:false}),biomeAt:()=> 'meadow'};
const monsters=createMonsters(new THREE.Group(),{field,heightAt:()=>0,sitesNear:()=>[],inDungeon:false},{combat,groupChance:0,loot:{rollFor:()=>null}});
for(const e of ORE_ELEMENTALS){assert(MONSTERS[e.id]);assert(e.reward>8);const model=buildOreElemental(e.id);model.update(.1);model.setAnim('swing');model.update(.1);model.setAnim('die');for(let i=0;i<30;i++)model.update(.1);assert(model.dieDone);model.dispose();const spawned=monsters.spawnEncounter(e.id,0,0);assert(spawned);const mon=monsters.all().find(m=>m.id===e.id);combat.kill(mon.actor,{id:'player',kind:'player',bonuses:{}});const corpse=monsters.corpses().find(c=>c.id===e.id&&c.loot);const item=corpse?.loot?.items.find(i=>i.base===e.ore+'_ore');assert.equal(item?.count,e.reward,'ore elemental reward stays on its corpse');}
