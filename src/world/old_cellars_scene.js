...
export function furnishOldCellars(built,L){
 ...
 for(const r of L.rooms){
  if(r.id===L.landmark.roomId)continue;
 ...
 root.add(new T.HemisphereLight(0xb1c2d9,0x343341,.58));
 ...
 const landmark=furnishBlenderCellar(built,L);built.landmark=landmark;built.ready=landmark.ready;
 for(const anchor of landmark.anchors){
 ...
 const magic=createCellarMagic(root,landmark.anchors);
 ...
 built.dispose=()=>{if(disposed)return;disposed=true;for(const fx of effects)fx.dispose();magic.dispose();landmark.dispose();built.raid?.model.dispose();dispose();};
