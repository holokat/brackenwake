...
// -------------------------------------------------------------------- slots
{
  check('there are twelve slots', SLOTS.length === 12, SLOTS.join(' '));
  check('no slot is named twice', new Set(SLOTS).size === 12);
  for (const s of ['head', 'shoulders', 'chest', 'hands', 'waist', 'legs', 'feet', 'neck', 'ring1', 'ring2', 'mainHand', 'offHand']) {
    if (!SLOTS.includes(s)) check(`slot ${s} exists`, false);
  }
  check('no retired armour or ranged slot remains',
    !['wrists', 'back', 'outfit', 'ranged'].some((s) => SLOTS.includes(s)),
    SLOTS.join(' '));
  check('every documented slot is present', SLOTS.length === 12);
}
...
// ------------------------------------------------------------- armour tiers
{
  ...
  check('seven visible armour pieces fill the requested slots', ARMOR_PIECES.map((p) => p.id).join(' ') === 'head shoulders chest hands waist legs feet',
    ARMOR_PIECES.map((p) => p.id).join(' '));
  check('the seven pieces carry the old nine shares of AR', ARMOR_PIECES.reduce((n, p) => n + p.arMul, 0) === 9, String(ARMOR_PIECES.reduce((n, p) => n + p.arMul, 0)));
  ...
  check('a full plate suit is AR 108', plate.reduce((s, b) => s + b.ar, 0) === 108);
  ...
  check('leather burdens it a tenth', baseFor('leather_chest').castBurden === 0.1);
  ...
  check('all armour pieces carry the burden of their tier',
    ARMOR_TIERS.every((t) => setOf(t.id).every((b) => b.castBurden === t.castBurden)), '6 bases');
  check('all six materials give seven pieces', ARMOR_TIERS.every((t) => setOf(t.id).length === 7 && setOf(t.id).every(Boolean)), '42 bases');
}
...
// -------------------------------------------------------------- can you wear it
{
  ...
  check('AR through the penalty is 18 at 60 STR', armourOf(makeItem({ base: 'plate_chest' }), { STR: 60 }) === 18);
  check('AR through the penalty is 36 at 75 STR', armourOf(makeItem({ base: 'plate_chest' }), { STR: 75 }) === 36);
...
// -------------------------------------------------------------------- slots
{
  ...
  check('a chest piece goes to the chest slot', equipSlotFor('plate_chest') === 'chest');
