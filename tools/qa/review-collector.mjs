...
const origin=process.env.BRACKENWAKE_QA_ORIGIN || 'http://localhost:5317';
if(!/^http:\/\/(localhost|127\.0\.0\.1):\d+$/.test(origin))throw Error('The review origin must be a local HTTP server.');
