...
const bossDetails=()=>b.monsters.all().filter(m=>/Morva|Sexton|Cinder|Ilex|Voss|Aster|Oram|Vharos/i.test(m.name||m.actor.name||'')).map(m=>({name:m.name||m.actor.name,pos:{...m.actor.pos},loaded:!!m.model?.loaded,phase:m.phase,pending:m.cellarCombat?.pending?.length||0}));
